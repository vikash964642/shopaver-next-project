    ...
    <>
        ...
        <div className="flex items-center space-x-6 relative">
          {navLinks.map((link) =>
            ...
            className={`ml-1 transition-transform duration-300 flex items-center ${
              desktopDropdown === link.name ? "rotate-180" : "rotate-0"
            }`}
          >
            <ion-icon name="chevron-down" class="text-[20px]"></ion-icon>
          </span>
                </span>
      ...
      <div className="flex items-center gap-[9px]">
        <Link
          ...
          <button className="flex items-center" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
          {/* {mobileMenuOpen ? "✕" : "☰"} */}
          <ion-icon class="text-[25px] text-[#1D1D1D]"  name={mobileMenuOpen ? "close" : "menu"}></ion-icon>
        </button>
