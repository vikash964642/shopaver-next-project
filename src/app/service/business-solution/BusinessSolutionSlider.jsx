...
const CardData = [
  {
    ...
    paragraph: "Create bills with a simple, error-free billing experience designed for busy retail counters, making checkout fast and seamless.",
  },
  {
    image: "/landingPage/BusinessSolutionSliderImg2.webp",
    heading: "Quick Billing",
    subHeading: "Built for Speed at the Counter",
    paragraph: "Generate invoices instantly in seconds without the need for inventory updates and accept  payments easily with no delays or mistakes.",
  },
  {
    image: "/landingPage/BusinessSolutionSliderImg3.webp",
    heading: "Inventory Management",
    subHeading: "Always Know Your Stock",
    paragraph: "Track inventory in real time across products and outlets, prevent stockouts, and manage  supplies with confidence.",
  },
  {
    image: "/landingPage/BusinessSolutionSliderImg4.webp",
    heading: "Digital Khata",
    subHeading: "Smarter Credit Management",
    paragraph: "Maintain complete customer credit and debit records digitally, track dues easily, and ensure  timely collections.",
  },
  {
    image: "/landingPage/BusinessSolutionSliderImg5.webp",
    heading: "Online Store",
    subHeading: "Sell Beyond Your Physical Shop",
    paragraph: "Launch your online store instantly and manage orders, payments, and inventory from single  smart dashboard.",
  },
  {
    image: "/landingPage/BusinessSolutionSliderImg6.webp",
    heading: "WhatsApp Commerce",
    subHeading: "Turn Chats into Sales",
    paragraph: "Turn WhatsApp conversations into confirmed orders by sharing catalogues, receiving  payments, and fulfilling faster.",
  },
  {
    image: "/landingPage/BusinessSolutionSliderImg7.webp",
    heading: "WhatsApp Marketing",
    subHeading: "Increase More & Repeat Sales",
    paragraph: "Engage customers with targeted campaigns, personalized offers, and timely updates to boost  repeat business.",
  },
];
function BusinessSolutionSlider({ data = [] }) {
  // const filteredData = allinone.filter(
  //   (item) => item.allInOneHeading && item.allInOneSubHeading && item.allInOneDescription 
  // );
 const filteredData = [...data];
  ...
  useLayoutEffect(() => {
    ...
    end: () => "+=" + sectionRef.current.offsetHeight * (filteredData.length - 1),
      pin: true,
      scrub: 1,
      pinSpacing: true,
      onUpdate: (self) => {
              ...
              <div className="flex flex-col md:flex-row items-stretch max-[360px]:px-[15px] max-[400px]:px-[25px] px-10 md:px-0 ">
                ...
                <div className=" md:w-[50%] flex justify-center items-center">
                  <Image
                    ...
                    className="lg:w-[400px] lg:h-[400px]" alt={card.heading} width={400} height={400}
                  ></Image>
...
>
  {filteredData.map((card, index) => (
    <SwiperSlide key={index} className="!w-[250px]">
      ...
      <div className="flex justify-center">
          <Image src={"/landingPage/BusinessSolutionSliderImg1.webp"} alt={card.heading} width={234} height={234} className="w-full aspect-square object-cover"  />
      </div>
        <h2 className="pt-[14.68px] text-[20.55px] sm:text-[22px] font-semibold text-[#5801B7]">{card.heading}</h2>
        <p className="pt-[5px] text-[16.14px] sm:text-[18px] font-medium text-[#0A0A0A]">{card.subHeading}</p>
        <p className="pt-[2px] text-[10.275px] sm:text-[12px] font-normal text-[#5F5F5F]">{card.description}</p>
      </div>
    </SwiperSlide>
  ))}
</Swiper>
