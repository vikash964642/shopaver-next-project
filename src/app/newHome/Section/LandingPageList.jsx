...
function LandingPageList() {
  ...
  return (
    <section className="bg-[#E3E3FF] mt-[7.5rem] py-[2.5625rem] lg:py-[3.875rem]">
      <div className="max-w-[115.625rem] px-[1.375rem] xl:px-0 mx-32">
        {/* TITLE */}
        <h2 className="text-[#5801B7] text-center text-[1.875rem] lg:text-[2.5rem] font-bold lg:font-medium font-bricolage">
    ...
    <SwiperSlide key={groupIndex}>
      <div className="grid grid-cols-2 gap-[1.25rem]">
        {listData.slice(groupIndex * 4, groupIndex * 4 + 4).map((item, index) => (
          <div key={index} className="h-[15.625rem]">
            <Card item={item} />
          </div>
        ))}
      </div>
...
function Card({ item }) {
  ...
  return (
    <div className="flex items-stretch gap-[0.90625rem] lg:gap-[2.1875rem] bg-white rounded-[1.591875rem] border border-[#EBD9FF] py-[0.8125rem] pl-[1.28125rem] pr-[0.79375rem] h-full">
      {/* LEFT CONTENT */}
      <div className="flex flex-col w-[50%] justify-center">
     
          <p className="text-[#3C3939] text-[1.125rem] font-medium break-words line-clamp-2 font-bricolage">
            {item.title}
          </p>
         <p className="pt-[0.34375rem] text-[#666] text-[1rem] font-normal leading-normal font-dm-sans line-clamp-5">{item.description}</p>


        <button

          className="mt-[1.25rem] rounded-[1.875rem] bg-[#E6D2FF] w-[6.0625rem] h-[1.875rem] flex justify-center items-center gap-[0.28125rem]"
        >
          <p className="text-[#5801B7] font-normal lg:font-medium text-[0.656rem] lg:text-[0.75rem]">Read More</p>
          <div className="h-[1.125rem] w-[1.125rem] rounded-full bg-[#FFF] flex justify-center items-center ">
  ...
  <path d="M10.3536 4.03519C10.5488 3.83993 10.5488 3.52335 10.3536 3.32809L7.17157 0.146107C6.97631 -0.0491549 6.65973 -0.0491549 6.46447 0.146107C6.2692 0.341369 6.2692 0.657952 6.46447 0.853214L9.29289 3.68164L6.46447 6.51007C6.2692 6.70533 6.2692 7.02191 6.46447 7.21718C6.65973 7.41244 6.97631 7.41244 7.17157 7.21718L10.3536 4.03519ZM0 3.68164L-4.37114e-08 4.18164L10 4.18164L10 3.68164L10 3.18164L4.37114e-08 3.18164L0 3.68164Z" fill="#5801B7"/>
      ...
      <div className="w-[50%] flex justify-end items-center">
        <Image
          ...
          width={218}
          height={227}
          className="rounded-[0.6875rem] w-full h-full object-contain"
        />
      </div>
    </div>
  );
